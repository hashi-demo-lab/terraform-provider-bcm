...
"""
API Exploration Script: CMPart getPartitions

Purpose: Call BCM API endpoint {"service": "cmpart", "call": "getPartitions"}
...
Usage:
    ...
    python3 sampleRest/cmpart-get-partitions.py
"""
...
def main():
    ...
    # Step 2: Call cmpart.getPartitions
    print("\nCalling cmpart.getPartitions API...")
    ...
    try:
        ...
        print("\n" + "=" * 80)
        print("API RESPONSE: cmpart.getPartitions")
        print("=" * 80)
        ...
        if isinstance(response_data, list):
            print(f"\n✓ Response is a JSON array with {len(response_data)} partition(s)")

            if response_data:
                print("\nFirst partition object keys:")
                for key in sorted(response_data[0].keys()):
                    value = response_data[0][key]
                    value_type = type(value).__name__
                    # Show first 100 chars of value for strings
                ...
                print("\n⚠ WARNING: No partitions found in BCM cluster")
                print("Acceptance tests require at least one partition to exist")
        else:
